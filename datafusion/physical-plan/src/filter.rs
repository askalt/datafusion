...
use crate::projection::{
    EmbeddedProjection, OptionProjectionRef, ProjectionExec, ProjectionExpr,
...
pub struct FilterExec {
    ...
    projection: OptionProjectionRef,
...
impl FilterExecBuilder {
    ...
    pub fn apply_projection(
        ...
        self.projection = projection.apply_projection(&self.projection)?;
        Ok(self)
    }
    ...
    pub fn build(self) -> Result<FilterExec> {
        ...
        can_project(&self.input.schema(), self.projection.as_ref())?;
...
impl FilterExec {
    ...
    /// Projection
    pub fn projection(&self) -> &OptionProjectionRef {
        &self.projection
    }
...
mod tests {
    ...
    async fn test_builder_projection_composition() -> Result<()> {
        ...
        assert_eq!(filter.projection(), Some(&[0, 3]));
    ...
    async fn test_builder_projection_composition_none_clears() -> Result<()> {
        ...
        assert_eq!(filter.projection(), None::<&[usize]>);
